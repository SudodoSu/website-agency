...
function Article({articles, background, backgroundOut, hover}) {


    return(
        <>
            {articles.map((article) => (
                <article key={article.num} className={`our-speciallization-articles > article ${(hover[article.num]) ? "growArticle" : ""}`} onMouseEnter={() => background(article.num)} onMouseLeave={() => backgroundOut(article.num)}>
